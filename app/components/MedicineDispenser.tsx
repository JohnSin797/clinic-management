...
interface Medicine {
    medicine: { id: string, quantity: number }[];
    record: string;
}
...
const MedicineDispenser: FC<Props> = ({ isHidden, toggle, medicines, record }) => {
    ...
    const addItem = () => {
        setMedicineForm((prevForm) => ({
            ...prevForm,
            medicine: [...prevForm.medicine, { id: '', quantity: 0 }],
        }));
    };
    ...
    const handleMedicineChange = (index: number, key: 'id' | 'quantity', value: string | number) => {
        setMedicineForm((prevForm) => {
                            ...
                            medicineForm.medicine.map((med, index) => {
                                ...
                                return (
                                    <div key={index} className="w-full flex justify-center items-center gap-2 mb-2">
                                        ...
                                        <input
                                            type="number"
                                            className="p-2 rounded ring-2 ring-black text-sm w-1/3"
                                            value={med.quantity}
                                            onChange={(e) =>
